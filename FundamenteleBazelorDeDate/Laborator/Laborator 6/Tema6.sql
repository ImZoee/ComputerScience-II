...
insert into PRODUSE(idprodus, denumire, descriere) values
    (6,'Monitor','Monitor Samsung LED 24"'),
    (7,'Monitor','Monitor IPS LG 21"'),
    (8,'Monitor','Monitor LED Lenovo 20"')
...
insert into VANZARI(idvanzare, idprodus, idClient, idvanzator, datavanzare, nrproduse) values
    (6,6,3,4,'2025-01-22',70),
    (7,7,2,2,'2025-01-01',100),
    (8,8,1,3,'2024-12-01',100)

select * from ANGAJATI A inner join SECTII S on A.IdSectie = S.IdSectie
select * from ANGAJATI A left join SECTII S on A.IdSectie = S.IdSectie
select * from ANGAJATI A right join SECTII S on A.IdSectie = S.IdSectie
select * from ANGAJATI A full outer join SECTII S on A.IdSectie = S.IdSectie

select * from ANGAJATI A join ANGAJATI A2 on A.IdSectie = A2.IdSectie
and A.Salariu < A2.Salariu

select * from SECRETARE S join SECRETARE S2 on S.VitezaRedactare > S2.VitezaRedactare

select * from ANGAJATI A join ANGAJATI A2 on A.IdSectie = A2.IdSectie
and  A.IdAngajat <> A2.IdAngajat

select * from CLIENTI C join CLIENTI C2 on C.Adresa = C2.Adresa and C.IdClient <> C2.IdClient
